...
Hàm trong js được chia làm 2 loại chính và chia theo chức năng của nó
Function declaration | Function Expression
Nhưng nếu phân loại một cách đầy đủ theo cú pháp và tính chất hoạt động, chúng ta có 4 loại hàm chính:
...
---
#### Function Declaration
nó hosting, có thể gọi trước khai báo sau 
...
Function Expression là gán một hàm cho một biến.Và nó không có tính hoisted.
...
Trong JavaScript (ngoại trừ Arrow Function), mọi hàm đều có sẵn một đối tượng giống mảng tên là arguments chứa tất cả các giá trị được truyền vào.
...
demoArguments(1, 2, 3, 4, 5); // Output: 15
...
là hàm k cần đặt tên , chạy ngay sau khi tạo liền , k tái sử dụng được luôn
```js
(function handle3() {
  let a = 10;
  let b = 20;
...
// setTimeout(handle3,3000) // ko có mở ngoặc nha vì mở ngoặc là nó sẽ chạy luôn
...
- Trong Function Declaration và Function Expression, 'this' tham chiếu đến đối tượng gọi hàm.
- Trong Arrow Function, 'this' tham chiếu đến ngữ cảnh nơi nó được định nghĩa, không phải đối tượng gọi hàm.
...
Trong mỗi thuộc tính sẽ có 1 cặp accessor property là get và set để lấy và gán giá trị cho thuộc tính đó
```js
let person2 = {
  fullname: "Điệp đẹp trai",  
  get getName() {
